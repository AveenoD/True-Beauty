...
export default function CartPage() {
  ...
  const { items: cartItems, loading: cartLoading, error: cartError, updateItemQuantity, removeItem } = useCart();
  ...
  useEffect(() => {
    ...
    if (!appliedFromStorage && typeof window !== 'undefined') {
      ...
      if (available.length > 0) {
        ...
        const minimalCart: MinimalCartItem[] = (cartItems || []).map((item) => ({
          // coupons util only needs numeric id, so use a stable hash of productId
          id: Math.abs(item.productId.split('').reduce((acc, ch) => (acc * 31 + ch.charCodeAt(0)) | 0, 7)),
          price: item.price,
        ...
        const result = validateCoupon(
          preferred.code,
          minimalCart,
          () => ({ id: 0, category: 'general', price: 0, originalPrice: 0 } as any),
          user
        );
        if (result.ok) {
  ...
  const changeQuantity = async (itemId: string, delta: number) => {
    ...
    if (!current) return;
    const nextQty = Math.max(0, (current.quantity || 1) + delta);
  ...
  const handleApplyCoupon = () => {
    ...
    const result = validateCoupon(couponCode, minimalCart, () => ({ id: 0, category: 'general', price: 0, originalPrice: 0 } as any), user);
        ...
        <div className="container mx-auto max-w-4xl">
          ...
          {cartLoading ? (
            <div className="text-center py-16">
              ...
              <h2 className="text-xl font-semibold text-gray-800 mb-2">Loading cart…</h2>
              <p className="text-gray-600">Fetching latest items from the server.</p>
            </div>
          ) : cartError ? (
            <div className="text-center py-16">
              <ShoppingBag className="w-20 h-20 text-rose-200 mx-auto mb-4" />
              <h2 className="text-xl font-semibold text-gray-800 mb-2">Couldn’t load cart</h2>
              <p className="text-gray-600 mb-6">{cartError}</p>
              <button
                type="button"
                onClick={() => window.location.reload()}
                className="inline-block bg-gradient-to-r from-[#FF3C8C] to-[#FF0066] text-white px-6 py-2.5 rounded-lg font-medium hover:opacity-95 transition-colors"
              >
                Retry
              </button>
            </div>
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-600 mb-6">Your cart is saved to your account. Sign in to view it.</p>
              <Link
              ...
              <div className="space-y-4 mb-8">
                {cartItems.map((item) => (
                  <div key={item.id} className="bg-white rounded-xl shadow-sm border border-rose-100 p-4 md:p-5 flex gap-4">
                    <img
                      src={item.product.image || '/images/products/dayCream.png'}
